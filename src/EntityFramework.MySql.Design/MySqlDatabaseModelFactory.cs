    ...
    public class MySqlDatabaseModelFactory : IDatabaseModelFactory
    {
            ...
            [NotNull] string connectionString, [NotNull] TableSelectionSet tableSelectionSet)
        {
            ...
            using (_connection = new MySqlConnection(connectionString))
            {
                ...
                {
                    MySqlDmlParser.ParseTableDefinition(table, _tableDefinitions[table.Name]);
                }
        ...
        private void GetMySqlMaster()
        {
            var command = _connection.CreateCommand();
            command.CommandText = "SELECT type, name, sql, tbl_name FROM MySql_master ORDER BY type DESC";

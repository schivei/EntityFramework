    ...
    public class MySqlMigrationsSqlGenerator : MigrationsSqlGenerator
    {
        ...
        protected override void Generate(CreateTableOperation operation, IModel model, RelationalCommandListBuilder builder)
        {
            ...
            if (operation.PrimaryKey?.Columns.Length == 1)
            {
                ...
                if (columnOp != null)
                {
                    columnOp.AddAnnotation(MySqlAnnotationNames.Prefix + MySqlAnnotationNames.InlinePrimaryKey, true);
                    if (!string.IsNullOrEmpty(operation.PrimaryKey.Name))
                    {
                        columnOp.AddAnnotation(MySqlAnnotationNames.Prefix + MySqlAnnotationNames.InlinePrimaryKeyName, operation.PrimaryKey.Name);
                    }
            ...
            RelationalCommandListBuilder builder)
        {
            ...
            var inlinePk = annotatable[MySqlAnnotationNames.Prefix + MySqlAnnotationNames.InlinePrimaryKey] as bool?;
            if (inlinePk == true)
            {
                var inlinePkName = annotatable[
                    MySqlAnnotationNames.Prefix + MySqlAnnotationNames.InlinePrimaryKeyName] as string;
                ...
                var autoincrement = annotatable[MySqlAnnotationNames.Prefix + MySqlAnnotationNames.Autoincrement] as bool?;
        ...
        {
            throw new NotSupportedException(MySqlStrings.InvalidMigrationOperation);
        }
        ...
        {
            throw new NotSupportedException(MySqlStrings.SchemasNotSupported);
        }

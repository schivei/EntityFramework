...
{
    public abstract class MySqlE2ETestBase : E2ETestBase
    {
        public MySqlE2ETestBase(ITestOutputHelper output)
            : base(output)
        {
        }
        ...
        {
            using (var testStore = MySqlTestStore.GetOrCreateShared("OneToOne" + DbSuffix).AsTransient())
            {
...
");
                ...
                var results = await Generator.GenerateAsync(new ReverseEngineeringConfiguration
                {
                    ...
                    ProjectPath = "testout",
                    ProjectRootNamespace = "E2E.MySql",
        ...
        {
            using (var testStore = MySqlTestStore.GetOrCreateShared("OneToMany" + DbSuffix).AsTransient())
            {
                testStore.ExecuteNonQuery(@"
...
");
        ...
        {
            using (var testStore = MySqlTestStore.GetOrCreateShared("ManyToMany" + DbSuffix).AsTransient())
            {
                testStore.ExecuteNonQuery(@"
...
");
        ...
        {
            using (var testStore = MySqlTestStore.GetOrCreateShared("SelfRef" + DbSuffix).AsTransient())
            {
    ...
    FOREIGN KEY (SelfForeignKey) REFERENCES SelfRef (Id)
        ...
        {
            using (var testStore = MySqlTestStore.CreateScratch())
            {
        ...
        {
            using (var testStore = MySqlTestStore.GetOrCreateShared("NoPrincipalPk" + DbSuffix).AsTransient())
            {
...
");
        ...
        {
            using (var testStore = MySqlTestStore.GetOrCreateShared("FkToAltKey" + DbSuffix).AsTransient())
            {
                testStore.ExecuteNonQuery(@"
CREATE TABLE User (
    Id INTEGER PRIMARY KEY,

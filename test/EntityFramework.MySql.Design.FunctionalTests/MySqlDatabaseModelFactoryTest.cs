...
using Microsoft.Data.Entity.MySql.FunctionalTests;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Logging;
using Xunit;

namespace EntityFramework.MySql.Design.FunctionalTests
{
    public class MySqlDatabaseModelFactoryTest
    {
        ...
        public MySqlDatabaseModelFactoryTest()
        {
        ...
        public void It_loads_column_types()
        {
            ...
            Assert.Equal("Column Types", entityType.MySql().TableName);

            Assert.Equal("text", entityType.FindProperty("col1").MySql().ColumnType);
            Assert.Equal(typeof(string), entityType.FindProperty("col1").ClrType);

            Assert.Equal("unsigned big int", entityType.FindProperty("col2").MySql().ColumnType);
        ...
        public void It_loads_default_values()
        {
            ...
            Assert.Equal("\"dev\"", entityType.FindProperty("occupation").MySql().GeneratedValueSql);
            Assert.Equal("2", entityType.FindProperty("pay").MySql().GeneratedValueSql);
            Assert.Equal("current_timestamp", entityType.FindProperty("hiredate").MySql().GeneratedValueSql);
            Assert.Equal("100 + 19.4", entityType.FindProperty("iq").MySql().GeneratedValueSql);
            Assert.Null(entityType.FindProperty("name").MySql().GeneratedValueSql);
        }
        ...
        public void It_loads_primary_key(string def, string[] keys)
        {
            ...
            Assert.NotNull(entityType);
            Assert.Equal(keys, entityType.GetKeys().First().Properties.Select(p => p.MySql().ColumnName).ToArray());
        }
        ...
        public void It_gets_unique_indexes(string[] columns, string create)
        {
            ...
            Assert.Equal(
                columns,
                idx.Properties.Select(c => c.MySql().ColumnName).ToArray());

            var props = columns.Select(c => entityType.GetProperties().First(p => p.MySql().ColumnName == c)).ToArray();
